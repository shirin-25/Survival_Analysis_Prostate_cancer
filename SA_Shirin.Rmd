...
```
```{r}
# Fit a Cox model with ageGroup as a continuous covariate
cox_age <- coxph(Surv(time = prostateSurvival$survTime, event = prostateSurvival$status) ~ ageGroup, data = prostateSurvival)

# Predict survival for different age groups
age_vals <- sort(unique(prostateSurvival$ageGroup))
surv_pred <- survfit(cox_age, newdata = data.frame(ageGroup = age_vals))

# Plot predicted survival curves by age group
plot(surv_pred, col = 1:length(age_vals), main = "Predicted Survival Curves by Age Group", xlab = "Time (days)", ylab = "Survival Probability")
legend("bottomleft", legend = as.character(age_vals), col = 1:length(age_vals), lty = 1)
